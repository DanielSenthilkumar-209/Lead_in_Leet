/** 1701.) There is a restaurant with a single chef. You are given an array customers, where customers[i] = [arrival i, time i]:
...
class Solution {
  public double averageWaitingTime(int[][] customers) {
    ...
    for (int[] c : customers) {
      current = Math.max(current, 1.0 * c[0]) + c[1];
      wait_time += current - c[0];
    }

    return 1.0 * wait_time / customers.length;
  }
}