/** 2751.) There are n 1-indexed robots, each having a position on a line, health, and movement direction.
...
That is the final heath of robot 1 (if survived), final health of robot 2 (if survived), and so on. 
...
class Solution {
  public List<Integer> survivedRobotsHealths(int[] positions, int[] healths, String directions) {
    ...
    Robot[] robot = new Robot[positions.length];
    ArrayList<Robot> stk = new ArrayList<>(); 

    for (int i = 0; i < positions.length; i++)
      robot[i] = new Robot(i, positions[i], healths[i], directions.charAt(i));

    Arrays.sort(robot, (a, b) -> a.position - b.position);
    ...
    for (Robot r : robot) {
      ...
      while (!stk.isEmpty() && stk.get(stk.size() - 1).direction == 'R' && r.health > 0) {
        if (stk.get(stk.size() - 1).health == r.health) {
          stk.remove(stk.size() - 1);
          r.health = 0;
        } 
        else if (stk.get(stk.size() - 1).health < r.health) {
          stk.remove(stk.size() - 1);
          r.health -= 1;
        } 
        else { 
          stk.get(stk.size() - 1).health -= 1;
          r.health = 0;
        }
      }
      if (r.health > 0)
